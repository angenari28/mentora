...
import { readonly } from '../shared/student.validation';
...
export class StudentDeleteComponent implements OnInit, IFormReadonly, IStudentCourseReset {
  ...
  readonly modalTitle = signal('Excluir Aluno');
  ...
  protected readonly studentForm = form(this.model, readonly, {
    submission: {
      action: async () => {
        ...
        return new Promise<void>((resolve, reject) => {
          this.userService.delete(this.studentId).subscribe({
            next: () => {
            ...
            error: (err) => {
              this.submitting.set(false);
              this.submitError.set('Erro ao excluir aluno. Tente novamente.');
              console.error(err);
              reject(err);
            },
          });
